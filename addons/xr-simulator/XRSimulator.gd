...
func _ready():
	...
	camera = origin.get_node("PlayerCamera")
...
func _input(event):
	...
	if event is InputEventMouseMotion:
		...
		if not is_any_controller_selected:
			"""
			rotate_device > the hand doesn`t follow the camera rotation
			rotate_camera_and_origin > the hand follow camera rotation
			"""
#			rotate_device(event, camera)
			rotate_camera_and_origin(event, camera, origin)
...
func rotate_camera_and_origin(event: InputEventMouseMotion, camera: Node3D, origin: Node3D):
	var motion = event.relative
	origin.rotate_y(motion.x * -device_x_sensitivity/1000)
	camera.rotate(camera.transform.basis.x, motion.y * -device_y_sensitivity/1000)
